...
const main = async () => {
  ...
  const oldContractAddress = config.SBT_ADDRESS;
  if (!oldContractAddress) {
    console.log('Please add old contract address before upgrade');
    return;
  }

  SBT = await ethers.getContractFactory('SBT');
  sbt = await upgrades.upgradeProxy(oldContractAddress, SBT, [
